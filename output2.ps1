...
if ($files -ne $null) {
    ...
    foreach ($result in $files) {
        #$result | Remove-Item
        #$result | Format-Table -Property Directory, Name, LastWriteTime
        #$files | Format-Table -Property Directory, Name, LastWriteTime
        #$result.FullName | Format-Table -Property Directory, Name, LastWriteTime -GroupBy Directory
    }
}
else {
